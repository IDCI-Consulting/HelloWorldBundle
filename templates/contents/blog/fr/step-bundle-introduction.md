...
Nous avons testé d'autres bundles existants, qui semblaient répondre à notre problématique client, mais ceux-ci ne nous pas satisfait. En bas de page, nous avons ajouté un tableau comparatif de ces différents bundles.

Ainsi, nous avons choisi de développer notre propre Bundle. En cela, nous avons pensé ce bundle générique et réutilisable.

Cet article a pour vocation de vous présenter IDCIStepBundle. Dans de prochains articles, nous rentrerons dans le vif du sujet et vous parlerons des spécifités de StepBundle au travers d'exemples concrets.

IDCIStepBundle permet de simplifier la création d'un parcours interactif à destination d'un internaute. Sa configuration se définie par un système de map (carte), de step (étape) et de path (chemin).
...
Enfin, ajoutons les spécificités de configuration de ce Bundle à notre application. Pour cela, éditons le fichier `app/config/config.yml` :
