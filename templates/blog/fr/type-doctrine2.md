...
Il est possible qu'à un moment donné de votre projet Symfony2, vous ayez rencontré la problématique
du stockage de médias en base de données.
Si oui, vous vous êtes forcément posé la question suivante : **Comment créer un nouveau type de données Doctrine2 avec Symfony2 ?**
...
Comme trame de cet article, nous allons tenter de créer une nouvelle entité Media qui sera stockée en base de données en type blob.
...
C'est dans ce dossier que vous allez pouvoir créer nos nouveaux types de données.
Pour notre exemple, nous allons réaliser une classe BlobType qui héritera de la classe AbstractPlatform.
Nous devons déclarer obligatoirement les fonctions suivantes :
...
Ces fonctions nous permettent de définir un comportement pour nos champs.
Nous pouvons par exemple avoir besoin de crypter un mot de passe avant de l'enregistrer en base de données.
...
Une fois notre type créé, il faut indiquer à Doctrine2 que celui-ci est disponible à l'utilisation.
Pour cela, il nous faut enregistrer celui-ci grace à la fonction registerDoctrineTypeMapping().
Celle-ci permet de charger notre nouveau type de données dans la fonction boot() du bootstrap de notre bundle :
...
Notre nouveau type est désormais configuré et prêt à l'emploi dans nos entités.


## Utiliser le nouveau type Doctrine2 dans une entity de notre bundle Symfony2

Pour terminer notre exemple d'ajout d'une entité Media persistée au format blob en base de données à notre projet,
voici ce à quoi pourrait ressembler celle-ci :
...
[nous contacter]({{ path('website_contact') }} "Contactez-nous").
